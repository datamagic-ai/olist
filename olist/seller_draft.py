import pandas as pd
import numpy as np
from olist.data_solution import Olist
...
class Seller:
    ...
    def get_active_dates(self):
        ...
        dates['months_active'] = np.ceil((dates['date_last_sale'] - dates['date_first_sale'])/np.timedelta64(1,'M'))
        dates['months_active'] = dates['months_active'].apply(lambda x: 1 if x ==0 else x)
    ...
    def get_review_score(self):
        ...
        reviews_df['cost_review'] = reviews_df['review_score'].map({5:0,
                                                                 4:0,
                                                                 3: 40,
                                                                 2: 50,
                                                                 1:100
        ...
        reviews_df.columns = ['seller_id', 'share_of_one_stars',
                              'share_of_five_stars', 'review_score' , 'cost_review']
    ...
    def get_training_data(self):
        ...
        df['revenue'] = df['months_active']*80 + df['sales']*0.1
        df['profit'] =  df['revenue'] - df['cost_review']
